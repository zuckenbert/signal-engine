...
| --- | --- | --- |
| `collect-records.yml` | `schedule` (cron) + manual | Roda o `official-records/collect.sh` (coleta determinística, sem LLM) e commita o dado cru em `official-records/_raw/`. Esse `_raw` versionado é o bastão pra rotina ler depois. |
...
A INTERPRETAÇÃO (as rotinas por eixo e por mercado, que enriquecem o ator, classificam fit, reconciliam contra a memória e escrevem o relatório) é trabalho de LLM e normalmente roda FORA do Actions, no agendador do seu próprio assistente de IA (a rotina/schedule dele).
...
Só os eixos DETERMINÍSTICOS têm um workflow de coleta como o `collect-records.yml`. Os eixos de JULGAMENTO (ex: `competitors/`, e o motor de descoberta `markets/`) não têm `collect.sh` nem Actions de coleta: a própria LLM coleta dentro da rotina, porque a coleta exige decidir o que buscar na web aberta. Veja a seção "Quem faz o que: braço e cérebro" no README da raiz.

## Adicionar um workflow de coleta pra um eixo determinístico novo

Copie o `collect-records.yml`, troque o `name`, o caminho do `collect.sh` e a pasta `_raw/` no passo de commit pelo do seu eixo novo.
