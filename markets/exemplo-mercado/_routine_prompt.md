# Rotina semanal: Mercado Exemplo (software financeiro no País X) (modo DESCOBERTA)
...
> Ao clonar este mercado, troque `Exemplo (software financeiro no País X)` e `exemplo-mercado` pelo nome real da pasta.
...
2. `markets/exemplo-mercado/_targets.md`: a SUITE DE SINAIS. Liste quais sinais estão LIGADOS (`[x]`). Só rode os ligados. Para cada um, anote queries-semente, termos/cargos que contam, janela e o filtro de ICP.
3. `markets/exemplo-mercado/_state.json`: a MEMÓRIA. Anote `last_run_date` e `themes[]`. Defina `startDate = last_run_date` (se null, é o 1o run: trate a janela só como a semana atual).
4. `markets/exemplo-mercado/_status.md`: confira o último run no Weekly Run Log.
...
Para cada sinal ligado, escreva `markets/exemplo-mercado/weeks/<WEEK_ID>/<sinal>.md` (siga o espírito do `SIGNAL_TEMPLATE.md` da raiz):
...
Escreva `markets/exemplo-mercado/weeks/<WEEK_ID>/consolidated.md`, que MESCLA todos os sinais do mercado:
...
- `markets/exemplo-mercado/_state.json`: `last_run_week = WEEK_ID`, `last_run_date = <hoje>`, e `themes[]` (NEW criados, ESCALATING/ONGOING atualizados, DORMANT/RESOLVED remarcados).
- `markets/exemplo-mercado/_themes.md`: tabela legível atualizada (apague a linha de exemplo se ainda estiver lá).
- `markets/exemplo-mercado/_status.md`: Dashboard + nova linha no Weekly Run Log.

## Passo 8: Commit e push

```
feat(markets/exemplo-mercado): weekly <WEEK_ID>
```
